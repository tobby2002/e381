...
def new_et_order(symbol, tf, fc, longshort, qtyrate_k, et_price, sl_price, tp_price, tp_price_w5, quantity,
                 wavepattern):
    params = [
        {
            "symbol": symbol,
    ...
    if r1 is not None:
        ...
        if r2:
            ...
            add_etsl_history(open_order_history, symbol, tf, fc, longshort, qtyrate_k, wavepattern, et_price,
                             sl_price, tp_price, tp_price_w5, quantity, order_et['orderId'], order_sl['orderId'],
                             order_et,
                             order_sl)
            return True
        else:
...
def moniwave_and_action(symbol, tf, trade_info, i=None):
    if run_mode:
        delta = (4 * fcnt[-1] + 1)
    ...
    for fc in fcnt:
        if 'long' in type:
            ...
            df_lows = c_compare_before_fractal(df_lows, condi_compare_before_fractal,
                                               condi_compare_before_fractal_shift)
                        ...
                        for rule in rules_to_check:
                            if wavepattern.check_rule(rule):
                                if c_check_valid_wave_in_history(open_order_history, symbol, tf, fc, wavepattern):
                                    if check_cons_for_new_etsl_order(open_order_history, df_all, symbol, tf, fc,
                                                                     longshort, wavepattern, ix, trade_info, qtyrate):
                                        available, quantity = c_balance_and_calc_quanty(symbol)
                                        if available:
                                            et_price, sl_price, tp_price, tp_price_w5 = get_trade_prices(
                                                open_order_history, symbol, longshort, wavepattern)
                                            try:
                                                qtyrate_k = get_qtyrate_k(trade_info, qtyrate)
                                                r = new_et_order(symbol, tf, fc, longshort, qtyrate_k, et_price,
                                                                 sl_price, tp_price, tp_price_w5, quantity, wavepattern)
                                                if r:
                                                    if plotview:
                                                        plot_pattern_m(df=df,
                                                                       wave_pattern=[
                                                                           [1, wavepattern.dates[0], id(wavepattern),
                                                                            wavepattern]],
                                                                       df_lows_plot=df_lows_plot,
                                                                       df_highs_plot=df_highs_plot,
                                                                       trade_info=None, title=str(symbol + ' %s ' % str(
                                                                'LONG' if longshort else 'SHORT') + '%sm %s' % (tf,
                                                                                                                fc) + ', ET: ' + str(
                                                                et_price)))
...
def add_etsl_history(open_order_history, symbol, tf, fc, longshort, qtyrate_k, w, et_price, sl_price, tp_price,
                     tp_price_w5, quantity, et_orderId, sl_orderId, order_et, order_sl):
    now = dt.datetime.now()
    history = {
        'id': et_orderId,
        'create_datetime': now,
...
def close_position_by_symbol(symbol, quantity, longshort, et_orderId):
    ...
    side = "LONG" if longshort else "SHORT"
    result_position_filtered = [x for x in positions if
        ...
        if history_new:
            for new in history_new:
                ...
                if r_get_open_orders_et_flg is True and r_get_open_orders_sl_flg is True and r_query_et[
                    ...
                    rt = close_position_by_symbol(symbol, quantity, longshort,
                                                  et_orderId)  # AFTER FORCE SL CLICK, REMAIN TP FILLED AND CLOSE POSI ET
                    if rt is not None:
                        logger.info(
                            symbol + ' IN ETSLETSL Ooooooooooo  AFTER FORCE SL CLICK, REMAIN TP FILLED AND CLOSE POSI ET OOOOOoooOOOOOoooOOOO' + str(
                                rt))
                        update_history_status(open_order_history, symbol, et_orderId, 'TP')  # TODO check win or lose
                    else:
                        print(symbol,
                              ' IN ETSLETSL OooooooooooOOOOOo  AFTER FORCE SL CLICK, REMAIN TP FILLED AND CLOSE POSI ET  FAIL ERROR  ooOOOOOoooOOOO' + str(
                                  rt))
                        logger.info(
                            symbol + ' IN ETSLETSL Ooooooooooo  AFTER FORCE SL CLICK, REMAIN TP FILLED AND CLOSE POSI ET FAIL ERROR OOOOOoooOOOOOoooOOOO' + str(
                                rt))
                ...
                #             symbol + ' IN ETSLETSL Ooooooooooo  AFTER PARTIALLY_FILLED, REMAIN TCLOSE POSI ET ERROR OOOOOoooOOOOOoooOOOO'+ str(rt))
                    ...
                    print('IN ETSL: ', symbol, str(r_get_open_orders_et_flg), str(r_get_open_orders_sl_flg),
                          r_query_et['status'], r_query_sl['status'])
        ...
        if history_tp:
            for tp in history_tp:
                ...
                # FORCE
                    ...
                    symbol, str(r_get_open_orders_tp_flg), str(r_get_open_orders_sl_flg), str(r_query_tp['status']),
                    str(r_query_sl['status'])))
                    update_history_status(open_order_history, symbol, et_orderId,
                                          'FORCE')  # AUTO TP FILLED  # TODO check win or lose

                elif r_get_open_orders_tp_flg is False and r_get_open_orders_sl_flg is True and r_query_tp[
                    'status'] == 'NEW' and r_query_sl['status'] == 'NEW':
                    print(
                        'IN TP: r_get_open_orders_tp_flg is False and r_get_open_orders_sl_flg is True and r_query_tp[status]==NEW and r_query_sl[status] == NEW',
                        symbol, True if r_get_open_orders_tp else False,
                    ...
                    'status'] == 'NEW' and r_query_sl['status'] == 'CANCELED':
                    close_position_by_symbol(symbol, quantity, longshort,
                                             et_orderId)  # FORCE SL, BEFORE CHECK TP CHEKER
...
def single(symbols, trade_info, *args):
    for symbol in symbols:
        ...
        for tf in timeframe:
            try:
                moniwave_and_action(symbol, tf, trade_info, i=i)
            except Exception as e:
