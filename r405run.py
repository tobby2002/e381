...
with open('r405config.json', 'r') as f:
    config = json.load(f)
...
def get_wave_prices(symbol, longshort, w):
    ...
    # reverse
    sl_price = set_price(symbol, w5, longshort)
    # reverse
    tp_price_w5 = set_price(symbol, w0, longshort)
...
def c_in_plrate_adaptive(o_his, symbol, longshort, w, t_mode):
    try:
        ...
        # reverse
        b_symbol = abs(sl_price - et_price) / abs(tp_price - et_price)  # one trade profit/lose rate
...
def c_current_price_in_reverse_zone(symbol, longshort, w):
    w0 = w.values[0]
    w1 = w.values[1]
    w2 = w.values[3]
    w3 = w.values[5]
    ...
    # reverse
    between_entry_target = et_price + abs(sl_price - et_price) * (et_zone_rate) if longshort else et_price - abs(
        sl_price - et_price) * (et_zone_rate)
    c_price = float(api_call('ticker_price', [symbol])['price'])
...
# reverse tp_price <--> sl_price
def c_current_price_in_zone_by_prices(symbol, longshort, et_price, tp_price):
    # reverse
    # between_entry_target = et_price + abs(sl_price - et_price) * (et_zone_rate) if longshort else et_price - abs(sl_price - et_price) * (et_zone_rate)
    c_price = float(api_call('ticker_price', [symbol])['price'])
    c_price_in_zone = (c_price > et_price and c_price < tp_price) \
        if longshort else \
        (c_price < et_price and c_price > tp_price)
...
def c_active_in_zone(df, longshort, w, et_price, sl_price, tp_price, tp_price_w5, price_base):
    ...
    if not df_active.empty:
        try:
            ...
            active_max_value = max(df_active.High.tolist(), default=sl_price) # reverse tp_price_w5 <--> sl_price
            active_min_value = min(df_active.Low.tolist(), default=sl_price)
        except Exception as e:
            logger.error('active_max_value:' + str(e))

        # reverse
        if price_base == 'REVERSE_PRICE':
            c_active_min_max_in_zone = (active_max_value < w_start_price and active_min_value > (w_end_price - o_fibo_value)) \
                                    if longshort else \
                                (active_min_value > w_start_price and active_max_value < (w_end_price + o_fibo_value))
...
def check_cons_for_new_etsl_order(o_his, df, symbol, tf, fc, longshort, w, et_price, sl_price, tp_price, tp_price_w5, ix, t_mode, t_info, qtyrate):
    try:
        ...
        if t_mode in ['REAL']:
            ...
            # for reverse
            if not c_active_in_zone(df, longshort, w, et_price, sl_price, tp_price, tp_price_w5, 'REVERSE_PRICE'):
                return False

            # if not c_current_price_in_zone(symbol, longshort, w):
            #     return False

            if not c_current_price_in_reverse_zone(symbol, longshort, w):
                return False
...
def c_allowed_intersect_count_df(df_active, line_price, cross_cnt):
    if intersect_idx:
        # df_active = df[w.idx_end:]  # w5 stick 포함 or --> maybe here, right -> No포함 (df[w.idx_end + 1:])
        if not df_active.empty:
            df_active['cross'] = df_active.apply(lambda x: 1 if x['High'] >= line_price and line_price >= x['Low'] else 0,
...
def real_trade(symbol, tf, fc, longshort, et_price, sl_price, tp_price, tp_price_w5, w, t_mode, o_his, df_lows_plot, df_highs_plot, df, t_info):
    ...
    if available:
        ...
        # c_interset, cross_cnt = c_allowed_intersect_df(df_active_with_w5_stick, et_price, 1)  # w5 stick 포함하여 계산, 0번 초과 제외
        # if not c_interset:
        #     return t_info, o_his
        # if cross_cnt <= 1:
        #     # check current price
        ...
        # reverse
        c_interset, cross_cnt = c_allowed_intersect_count_df(df_active_with_w5_stick, et_price, 1)  # w5 stick 포함하여 계산, 1번만 허용
        if not c_interset:
            return t_info, o_his
...
def test_trade(symbol, tf, fc, longshort, et_price, sl_price, tp_price, tp_price_w5, w, t_mode, o_his, df_lows_plot, df_highs_plot, wavepatterns, wavepattern_tpsl_l, wave_option_plot_l, wave_opt, ix, df, t_info):
    ...
    if dates:
        for k, close in enumerate(closes):
            ...
            c_positioning = (
                    position is False
                    and et_price >= detrends[k] > sl_price) if longshort else (
                    position is False and et_price <= detrends[k] < sl_price
            )
            c_profit = (position and trends[k] >= tp_price) if longshort else (position and trends[k] <= tp_price)
            c_stoploss = (position and detrends[k] <= sl_price) if longshort else (position and detrends[k] >= sl_price)
            c_stoploss_direct = (detrends[k] <= sl_price and trends[k] >= et_price) if longshort else (
                        detrends[k] >= sl_price and trends[k] <= et_price)
        ...
        if history_new:
            for new in history_new:
                ...
                if r_get_open_orders_et_flg is True and r_get_open_orders_sl_flg is True and r_query_et[
                    ...
                    rt, o_his = new_tp_order(symbol, tf, fc, longshort, tp_price, r_query_et['executedQty'], et_orderId, t_mode, o_his)
