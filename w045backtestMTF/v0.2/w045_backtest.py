...
def backtest_trade45(tf, df, symbol, fcnt, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, idx, wavepattern_l, wavepattern_tpsl_l):
    ...
    # target_price = w5 + abs(w5-w4)*10/20 if longshort else w5 - abs(w5-w4)*10/20
    target_price = w5
    entry_price = w4
        ...
        for i, close in enumerate(closes):
            ...
            if position is True:
                if c_profit or c_stoploss:
                    ...
                    # if w2_rate > 0.9:
                        s_11 = symbol + '           '
                        logger.info('%s %s %s x%s %s-%s %s %s %s %s %s %s' % (timeframe, s_11[:11], tf, leverage, period_days_ago, period_days_ago_till,  fcnt, 'L' if longshort else 'S', trade_inout_i[0][0][2:-3], '-',  trade_inout_i[1][0][8:-3], str(trade_stats)))
                              # , w2_rate,
