...
from models.helpers import plot_pattern_m, plot_pattern_n, plot_pattern
import datetime
import pandas as pd
import numpy as np
...
condi_active_next_bean_ok = config['default']['condi_active_next_bean_ok']
...
w_width_rate = config['default']['w_width_rate']
...
def print_condition():
    ...
    logger.info('condi_active_next_bean_ok: %s' % condi_active_next_bean_ok)
    ...
    logger.info('w_width_rate: %s' % w_width_rate)
...
def c_real_condition_by_fractal_index(df, fcnt, w, ix):  # real condititon by fractal index
    notreal = True
    try:
        real_condititon1 = True if (fcnt/2 < (w.idx_end - w.idx_start)) \
                                   and (w.idx_end - w.idx_start) < fcnt \
                                   and w.idx_start == ix else False
        real_condititon2 = True if df.iloc[ix + int(fcnt/2), 0] < (w.dates[-1]) \
                                   and (w.dates[-1])  <  df.iloc[ix + int(fcnt), 0] \
                                    else False
        if not (real_condititon1 and real_condititon2):
            return notreal
    except Exception as e:
        logger.error('c_real_condition_by_fractal_index e:%s' % str(e))
        return notreal
    return not notreal
...
def backtest_trade45(tf, df, symbol, fcnt, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, ix, wavepattern_l, wavepattern_tpsl_l, wave_option_plot_l):
    ...
    if closes:
        for i, close in enumerate(closes):
            ...
            c_stoploss_direct = (detrends[i] <= sl_price and trends[i] >= et_price) if longshort else (detrends[i] >= sl_price and trends[i] <= et_price)

            if c_stoploss_direct:
                position = True
                c_stoploss = True
                position_enter_i = [symbol, et_price, sl_price, tp_price, dates[i]]
                ...
                # if plotview:
                #     plot_pattern_n(df=df, wave_pattern=[[i, wavepattern.dates[0], id(wavepattern), wavepattern]],
            ...
            if position is True:
                ...
                if c_profit or c_stoploss:
                    ...
                    if c_stoploss:
                        ...
                        trade_inout_i = [position_enter_i[0], position_enter_i[1], position_enter_i[2], position_enter_i[3], position_enter_i[4], dates[i], longshort, 'LOSE']
                        out_price = sl_price
                    ...
                    if c_profit:
                        ...
                        trade_inout_i = [position_enter_i[0], position_enter_i[1], position_enter_i[2], position_enter_i[3], position_enter_i[4], dates[i], longshort, 'WIN']
                        out_price = tp_price
                        order_history.append(trade_inout_i)
                    ...
                    trade_stats = [len(trade_count), round(winrate, 2), asset_new, symbol, win_lose_flg, 'WIN' if win_lose_flg else 'LOSE', f_cum, tpi_cum, b_cum, f, tpi, b, b_symbol, str(qtyrate_k), str(round(pnl_percent, 4)), sum(pnl_history), sum(fee_history), round(asset_min, 2), round(asset_max, 2)]
                    ...
                    wavepattern_tpsl_l.append([ix, wavepattern.dates[0], id(wavepattern), wavepattern])
                    ...
                    # if w2_rate > 0.9:
                        ...
                        trade_out = trade_inout_i[5][8:-3]
                        logger.info('%s %s %s %s %s x%s %s-%s %s %s %s %s %s %s - %s' % (str(qtyrate_k), str(condi_compare_before_fractal_mode)+' :shift='+str(condi_compare_before_fractal_shift), timeframe, s_11[:11], tf, qtyrate_k, period_days_ago, period_days_ago_till,  fcnt, 'L' if longshort else 'S', trade_in, '-',  trade_out, str(trade_stats), str([et_price, sl_price, tp_price, out_price])))
                              # , w2_rate,
                    ...
                    #     if plotview:
                    #         plot_pattern_n(df=df, wave_pattern=[[i, wavepattern.dates[0], id(wavepattern), wavepattern]], df_lows_plot=df_lows_plot, df_highs_plot=df_highs_plot, trade_info=trade_info, title=str(
...
def moniwave_and_action(tf, symbol, trade_info, i=None):
    start_int = i
                            ...
                            if wavepattern.check_rule(rule):
                                if wavepattern in wavepatterns:
                                    ...
                                    if not check_has_same_wavepattern(symbol, fc, wavepattern_l, wavepattern):
                                        ...
                                        wavepattern_l.append([symbol, fc, ix, wavepattern.dates[0], id(wavepattern), wavepattern])
                                        ...
                                        if True:
                                            ...
                                            trade_info, trade_flg = backtest_trade45(tf, df_all, symbol, fc, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, ix, wavepattern_l, wavepattern_tpsl_l, wave_option_plot_l)
                                            ...
                                            if plotview and trade_flg:
                                                ...
                                                t = tf + '_' + str(i) + ':' + rule.name + ' ' + str(
                                                    wave_opt.values) + ' ' + str(
                                                    trade_info[0][-1] if trade_info[0] else [])
                                                plot_pattern_n(df=df_all, wave_pattern=wavepattern_tpsl_l,
                                                               df_lows_plot=df_lows_plot,
                                                               df_highs_plot=df_highs_plot,
                                                               trade_info=trade_info,
                                                               wave_options=wave_option_plot_l,
    ...
    return trade_info
    # except Exception as e:
    #     logger.info('moniwave_and_action Exception : %s ' % e)
...
def single(symbols, trade_info, *args):
    for symbol in symbols:
        for tf in timeframe:
            moniwave_and_action(tf, symbol, trade_info, i=i)
...
def account_trades_df(symbols):
    ...
    for s in symbols:
        try:
            ...
            if not df_trades.empty:
                ...
                df_trades['et_price_trade'] = df_trades.apply(lambda x: x['price'] if x['realizedPnl'] == 0 else 0, axis=1)
                ...
                df_trades['et_price_trade'] = df_trades['et_price_trade'].apply(pd.to_numeric)
                df_trades['et_price_trade'] = df_trades.groupby(['symbol_side_qty'])['et_price_trade'].transform('sum')
                ...
                if not df_orders.empty:
                    ...
                    df_orders['et_price'] = df_orders.apply(lambda x: float(x['price']) if x['status'] == 'FILLED' and (str(x['clientOrderId'])[:6] == 'limit_') else None, axis=1)
    ...
    df['uid'] = df.apply(lambda x: x['symbol'] + '_' + str(x['positionSide']) + '_' + str(x['et_price']) + '_' + str(x['out_price']), axis=1)
...
def trade_info_df(trade_info):
    ...
    if not df.empty:
        ...
        df['et_price'] = df.apply(lambda x: x['order_history'][0][1], axis=1)
        df['out_dt'] = df.apply(lambda x: x['order_history'][1][0], axis=1)
        df['out_price'] = df.apply(lambda x: x['order_history'][1][1], axis=1)
        df['longshort'] = df.apply(lambda x: 'LONG' if float(x['out_price']) < float(x['et_price']) and x['trade_count'] == 0 else 'SHORT', axis=1)
        df['uid'] = df.apply(lambda x: x['symbol'] + '_' + str(x['longshort'])+ '_' + str(x['et_price']) + '_' + str(x['out_price']), axis=1)
        # print(df)
    return df
