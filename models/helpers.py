...
# https://plotly.com/python/ohlc-charts/
def plot_pattern_k(symbol, df: pd.DataFrame, w, k, wave_pattern: WavePattern, wave_options=None, plot_list=None, df_lows_plot=None, df_highs_plot=None,trade_info=None, order_history=None, title: str = ''):
    ...
    if k + 1:
        ...
        df_active = df_active[:(k + 1)]
        active_data = go.Ohlc(x=df_active['Date'],
                       open=df_active['Open'],
                       high=df_active['High'],
                       low=df_active['Low'],
                       close=df_active['Close']
                        )
        pattern_l.append(active_data)

    # active next chart
    if k + 1:
        df_active_next = df[(w.idx_end + 1) + (k + 1):]
        active_next_data = go.Ohlc(x=df_active_next['Date'],
                       open=df_active_next['Open'],
                       high=df_active_next['High'],
                       low=df_active_next['Low'],
                       close=df_active_next['Close'],
                       increasing_line_color='cyan', decreasing_line_color='cyan'
                        )
        pattern_l.append(active_next_data)
