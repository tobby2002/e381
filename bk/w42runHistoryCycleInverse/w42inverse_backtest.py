...
def backtest_trade45(df, symbol, fcnt, longshort, df_lows_plot, df_highs_plot, wavepattern, trade_info, idx, wavepattern_l, wavepattern_tpsl_l):
    ...
    w_start_price = w.values[0]  # wave0
    ...
    sl_price = w5 + height_price * target_fibo if longshort else w5 - height_price * target_fibo  # 0.5 되돌림가정
    entry_price = w4
    target_price = w2
    # target_price = w1   # good
    ...
    if closes:
        for i, close in enumerate(closes):
            ...
            c_positioning = (position is False and detrends[i] <= entry_price and detrends[i] > target_price) if longshort else (position is False and trends[i] <= entry_price and detrends[i] > target_price)
            c_profit = (position and detrends[i] < target_price) if longshort else (position and detrends[i] > target_price)
            c_stoploss = (position and trends[i] >= sl_price) if longshort else (position and trends[i] <= sl_price)
            c_takeprofit_direct = (trends[i] >= entry_price and detrends[i] < target_price) if longshort else (trends[i] <= entry_price and trends[i] >= target_price)
            ...
            if c_takeprofit_direct:
                position = True
                c_profit = True
                position_enter_i = [dates[i], entry_price]
                logger.info('c_takeprofit_direct')
                open_order = {
                    'id': h_id,
            ...
            if position is True:
                if c_profit or c_stoploss:
                    ...
                    if True :
                    # if w2_rate > 0.9:
                        logger.info('%s %s %s %s %s %s %s' % (symbol, fcnt, 'S' if longshort else 'L', trade_inout_i[0][0][2:-3], '-',  trade_inout_i[1][0][11:-3], str(trade_stats)))
                              # , w2_rate,
